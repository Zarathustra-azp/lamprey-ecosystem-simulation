...
for i=1:k
   for n=1:100
    ...
    Kl =0.1*(s(n,i));
    qz =0.24*(1-z(n,i)/Kz);
    %alpha =0.56+(0.78-0.56)/(1+exp(0.6*(y(n))/Ky))*i/k;    
    ...
    P3=z(n,i)/sum;
    P4=s(n,i)/sum;
    sn(n,i)=-P1*log(P1)-P2*log(P2)-P3*log(P3)-P4*log(P4);%香农多样性(保留量纲差异，归一化前)

    t=sum/4;
    o=sqrt((x(n,i)-t)^2+(y(n,i)+M(n,i)+F(n,i)-t)^2+(z(n,i)-t)^2+(s(n,i)-t)^2);
    ICV(n,i)=t/o;%ICV稳定性(保留量纲差异，归一化前)
...
% t=(x__+y__+M__+F__+z__+s__)/4;
% o=sqrt((x__-t).^2+(y__+M__+F__-t).^2+(z__-t).^2+(s__-t).^2);
% ICV=t./o;%ICV稳定性(归一化后的)

%变量(x,y) y列代表改变性别比例的序列，x行代表同一个性别比例下随时间变化的序列
